...
(function () {
    ...
    unsafeWindow.fkeydown = false;
    let fKeyupTimeout = null;
    // plz change this if you want diff keybinding, but i like f for now
    unsafeWindow.addEventListener("keydown", (event) => {
        if (event.key.toLowerCase() !== "f" && event.key.toLowerCase() !== "c") return;

        // Cancel any pending keyup timeout
        if (fKeyupTimeout) {
            clearTimeout(fKeyupTimeout);
            fKeyupTimeout = null;
        }
    ...
    unsafeWindow.addEventListener("keyup", (event) => {
        ...
        // Restart the 1500ms timeout
        if (fKeyupTimeout) {
            clearTimeout(fKeyupTimeout);
        }

        fKeyupTimeout = setTimeout(() => {
            unsafeWindow.fkeydown = false;
            fKeyupTimeout = null;
        }, 500);
    });
    ...
    function renderX(dmg) {
        ...
        document.title = dmg;
        let level = 0;
        if (prevHp > 0 && killplyr && dmg >= 0) {
            killplyr = false;
            let newdmg = -prevHp;
            level = Math.round(-(newdmg/ 5));
            prevHp = 0;
        }
        else {
            level = Math.round(-(dmg / 5));
        }
        if (level >= 3) {
            unsafeWindow.BAWK.play("gun_dozenGauge_Octopus_fire", undefined, );
        }
        if (level < 1) return;
...
`;
    ...
    setTimeout(() => {
        // let origplay = unsafeWindow.BAWK.play;
        // unsafeWindow.BAWK.play = function (sound, volume, options) {
        //     origplay.apply(unsafeWindow.BAWK, [sound, volume, options]);
        //     if (sound == "on_killed_enemy") {
        //         killplyr = true;
        //     };
        // };
    }, 5000);
        ...
        if (ss && ss?.MYPLAYER && unsafeWindow?.extern?.inGame) {
            ...
            if (extract("playerInfo")) {
                ...
                if (player && player.distance && player[H.playing]) {
                    ...
                    renderX(Math.round(player[H.hp]) - prevHp, player.name);
                    prevHp = Math.round(player[H.hp]);
